fetch(chrome.runtime.getURL('urls.json'))
   ...
   .then(data => {
      ...
      const passwordJson = data.password;  // La contraseña desde el archivo JSON
      ...
      for (let i = 0; i < blacklist.length; i++) {
         if (url.includes(blacklist[i])) {
            ...
            submitButton.addEventListener('click', () => {
               const password = passwordInput.value.trim();  // Eliminar espacios extra de la contraseña ingresada
